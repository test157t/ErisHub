async function generateRaw({ prompt, systemPrompt, responseLength }) {
  if (typeof window !== 'undefined' && typeof window.nitralGenerateRaw === 'function') {
    return window.nitralGenerateRaw({ prompt, systemPrompt, responseLength })
  }
  return ''
}
...
function defaultStepsJson() {
  return '[{"pattern":"sine","duration":5000,"min":0,"max":100}]'
}
...
export function initModeBuilder({ NAME, PlayModeLoader, updateStatus, onModesUpdated }) {
  ...
  function getBasicReferenceBundle() {
    ...
    const sequenceExampleSteps = basicSequences[sequenceExampleName]?.steps || [
      { pattern: patternExampleName, min: 0, max: 100, duration: 5000, pause: 1000 },
    ]
