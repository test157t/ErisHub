...
import { runDeviceOutput, stopDeviceOutput } from './device_output.js'
  ...
  try {
    ...
    switch (cmd.type) {
      ...
      case 'stop':
        ...
        if (cmd.deviceIndex !== undefined) {
          await stopDeviceActions(cmd.deviceIndex)
        } else {
...
async function executeLinearCommand(device, startPos, endPos, duration) {
  ...
  await runDeviceOutput(device, deps.buttplug, 'PositionWithDuration', adjustedEnd / 100, duration)
}

async function sendIntensityToDevice(device, intensity, options = {}) {
  const { preferredActuator = 'Vibrate' } = options
  ...
  return await runDeviceOutput(device, deps.buttplug, preferredActuator, intensityValue)
}
  ...
  const executeStep = async () => {
    ...
    try {
      ...
      if (actionType === 'vibrate') {
        if (Array.isArray(step)) {
          ...
          if (vibrateAttrs) {
            for (let i = 0; i < Math.min(vibrateAttrs.length, step.length); i++) {
              ...
              await sendIntensityToDevice(targetDevice, normalized, { preferredActuator: 'Vibrate' })
            }
        ...
        await runDeviceOutput(targetDevice, deps.buttplug, 'Oscillate', intensity / 100)
      }
...
/**
 * Stop actions for a single device
 */
export async function stopDeviceActions(deviceIndex) {
  const connectedDevices = getLiveDevices()
  clearLinearStopTimer(deviceIndex)
  ...
  if (targetDevice) {
    try {
      await stopDeviceOutput(targetDevice)
    } catch (e) {
      console.error(`${deps.NAME}: Failed to stop device ${deviceIndex}:`, e)
    }
    markDevicePlaybackStop(deviceIndex, 'stopped')
  }
}
