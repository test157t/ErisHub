...
function hasMediaDomHelper() {
  try {
    const test = globalThis.$(document.createElement('div'))
    return typeof globalThis.$ === 'function' && typeof test.html === 'function' && typeof test.find === 'function'
  } catch (_e) {
    return false
  }
}

if (!hasMediaDomHelper()) {
  ...
  const appendContent = (target, html) => {
    if (!target) return
    if (typeof html === 'string') target.insertAdjacentHTML('beforeend', html)
    else if (html?.nodeType) target.appendChild(html)
  }
  globalThis.$ = (selector) => {
    ...
    const api = {
      ...
      addClass: (name) => { elements.forEach((item) => item?.classList?.add?.(...String(name).split(/\s+/).filter(Boolean))); return api },
      removeClass: (name) => { elements.forEach((item) => item?.classList?.remove?.(...String(name).split(/\s+/).filter(Boolean))); return api },
      text: (value) => { if (value === undefined) return element?.textContent || ''; elements.forEach((item) => { item.textContent = String(value) }); return api },
      html: (value) => { if (value === undefined) return element?.innerHTML || ''; elements.forEach((item) => { item.innerHTML = String(value) }); return api },
      val: (value) => { if (value === undefined) return element?.value; elements.forEach((item) => { item.value = value }); return api },
      is: (query) => query === ':visible' ? !!(element && element.getClientRects().length) : !!element?.matches?.(query),
      show: () => { elements.forEach((item) => { if (item?.style) item.style.display = '' }); return api },
      hide: () => { elements.forEach((item) => { if (item?.style) item.style.display = 'none' }); return api },
      toggle: (force) => { elements.forEach((item) => { if (item?.style) item.style.display = force ? '' : 'none' }); return api },
      slideUp: () => api.hide(),
      slideDown: () => api.show(),
      before: (content) => { elements.forEach((item) => { if (typeof content === 'string') item.insertAdjacentHTML('beforebegin', content); else if (content?.nodeType) item.parentNode?.insertBefore(content, item) }); return api },
      on: (type, delegatedOrListener, listener) => {
        const eventType = String(type).split('.')[0]
        elements.forEach((item) => {
          if (!item?.addEventListener) return
          if (typeof delegatedOrListener === 'string' && typeof listener === 'function') {
            item.addEventListener(eventType, (event) => {
              const target = event.target?.closest?.(delegatedOrListener)
              if (target && item.contains(target)) listener.call(target, event)
            })
          } else if (typeof delegatedOrListener === 'function') {
            item.addEventListener(eventType, delegatedOrListener)
          }
        })
        return api
      },
      prop: (name, value) => { if (value === undefined) return element?.[name]; elements.forEach((item) => { item[name] = value }); return api },
      attr: (name, value) => { if (value === undefined) return element?.getAttribute?.(name); elements.forEach((item) => item?.setAttribute?.(name, value)); return api },
      css: (name, value) => {
        if (typeof name === 'object') {
          elements.forEach((item) => Object.entries(name).forEach(([key, entryValue]) => item?.style?.setProperty?.(key, String(entryValue))))
          return api
        }
        if (value === undefined) return element ? getComputedStyle(element).getPropertyValue(name) : ''
        elements.forEach((item) => item?.style?.setProperty?.(name, String(value)))
        return api
      },
      data: (name) => element?.dataset?.[name],
      find: (selector) => { const found = []; elements.forEach((item) => { item.querySelectorAll(selector).forEach((child) => found.push(child)) }); return globalThis.$(found) },
      remove: () => { elements.forEach((item) => item?.remove?.()); return api },
      append: (content) => { elements.forEach((item) => appendContent(item, content)); return api },
    }
    return api
  }
}
...
function initMediaPlayer() {
  ...
  // Handle menu refresh button (data only; React handles the DOM)
  $("#intiface-menu-refresh-media-btn").on("click", refreshMenuMediaData)
  ...
  for (const device of devices) {
    ...
    if (device?.features && typeof device.features.values === 'function') {
      for (const feature of device.features.values()) {
        try {
          if (typeof feature?.hasOutput === 'function' && feature.hasOutput('Vibrate')) hasVibrate = true
          if (typeof feature?.hasOutput === 'function' && feature.hasOutput('Oscillate')) hasOscillate = true
          if (typeof feature?.hasOutput === 'function' && feature.hasOutput('Position')) hasLinear = true
          if (typeof feature?.hasOutput === 'function' && feature.hasOutput('HwPositionWithDuration')) hasLinear = true
        } catch (_e) {}
      }
    }
    if (typeof getDeviceType === 'function') {
      ...
      if (inferred === 'vibrator') hasVibrate = true
...
async function refreshFunscriptEditorList() {
  ...
  try {
    let files = (await getIntifaceAssetList('funscript'))
      .filter(filePath => normalizeAssetPathForMatch(filePath).startsWith('assets/intiface/funscripts/'))
    ...
    if (files.length === 0) {
      const listed = await listAssetFilesFromDirectoryListing('/assets/intiface/funscripts/', (name) => name.endsWith('.funscript'))
      files = listed.map((name) => ({
        name,
        path: `assets/intiface/funscripts/${name}`,
      }))
    }
...
async function refreshMenuMediaData() {
  ...
  try {
    ...
    if (mediaLibraryState.sortBy !== 'duration-desc' && mediaLibraryState.sortBy !== 'duration-asc') {
      ensureMediaDurationsLoaded().catch(() => {})
    }

    return mediaLibraryState.allFiles
  } catch (error) {
...
async function loadFunscript(videoPath) {
  try {
    ...
    const funscriptUrl = `/assets/intiface/funscripts/${encodeURIComponent(funscriptFilename)}`
  ...
  const panelHtml = `
    <div id="intiface-chat-media-panel" style="display: none; width: 100%; position: relative; margin-bottom: 10px; padding: 0;">
      <div id="intiface-chat-video-container" style="position: relative; width: 100%; line-height: 0;">
        <video id="intiface-chat-video-player" style="width: 100%; height: 320px; border-radius: 4px; background: #000; display: block;" controls>
...
async function resolveMediaFilename(requestedName) {
  ...
  if (knownFiles.length === 0) {
    const mediaExtensions = new Set(['.mp4', '.webm', '.ogv', '.mkv', '.avi', '.mov', '.mp3', '.wav', '.ogg', '.flac', '.m4a'])
    if (mediaExtensions.has(getFileExtension(cleaned)) && !cleaned.includes('/') && !cleaned.includes('\\')) {
      return { filename: cleaned, fuzzy: false }
    }
    return { filename: null, fuzzy: false }
  }
...
async function loadChatMediaFile(filename) {
  ...
  try {
    ...
    if (isAudioFile) {
      ...
      videoPlayer.css({ 'height': '320px', 'max-height': '320px', 'object-fit': 'contain', 'background': '#000', 'border': '', 'padding': '', 'margin': '' })
