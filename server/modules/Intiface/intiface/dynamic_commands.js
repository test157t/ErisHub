const SlashCommandParser = { commands: {}, addCommandObject(command) { this.commands[command.name] = command } }
class SlashCommand { static fromProps(props) { return props } }
const ARGUMENT_TYPE = { STRING: 'string', NUMBER: 'number' }
class SlashCommandNamedArgument { static fromProps(props) { return props } }
class SlashCommandEnumValue { constructor(value, description = null) { this.value = value; this.description = description } }
const macroSystem = { category: { STATE: 'state' }, registry: { hasMacro: () => false, registerMacro: () => {}, unregisterMacro: () => {} } }
const MacrosParser = { has: () => false, registerMacro: () => {}, unregisterMacro: () => {} }
const power_user = { experimental_macro_engine: false }
